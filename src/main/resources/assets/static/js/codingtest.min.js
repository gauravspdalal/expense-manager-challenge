...
angular.module("codingtest.partials",[]).run(["$templateCache",function(e){e.put("expenses-content.html",'<div ng-controller="ctrlExpenses"><h2>Expenses</h2><div bookmarklist="" id="bookmarks" class="section-list"></div><h3>Add an expense</h3><form name="expensesform" novalidate><ul><li ng-show="expensesform.apiError"><label></label><span class="error_message">A system error has occurred! Message: {{expensesform.errorcode}}</span></li><li><label>Date</label> <input type="text" ng-required="true" name="expenseDate" ng-model="newExpense.date" ui-date="dateOptions" placeholder="dd/mm/yyyy"></li><li ng-show="(expensesform.expenseDate.$dirty || expensesform.$submitted) && expensesform.expenseDate.$error.required"><label></label><span class="error_message">Date is required!</span></li><li><label>Amount</label> <input type="text" ng-required="true" ng-model="newExpense.amount" placeholder="00.00" ng-pattern="/^[0-9]+(\.[0-9]{1,10})?$/" ng-change="calculateVAT(newExpense.amount)" name="amountVal"></li><li ng-show="(expensesform.amountVal.$dirty || expensesform.$submitted) && (expensesform.amountVal.$error.required || expensesform.amountVal.$error.pattern)"><label></label><span class="error_message" ng-show="(expensesform.amountVal.$dirty || expensesform.$submitted) && (expensesform.amountVal.$error.pattern)">Invalid Amount!</span><span class="error_message" ng-show="(expensesform.amountVal.$dirty || expensesform.$submitted) && (expensesform.amountVal.$error.required)">Amount is required!</span></li><li><label>Calc VAT Amount</label><input type="text" ng-model="newExpense.vatAmount" placeholder="00.00" ng-disabled="true"/></li><li><label>Reason</label><div auto-resize-form="" resize-watches="newExpense.reason" resize-sources="textarea" class="auto-resize"><textarea ng-required="true" ng-model="newExpense.reason" ng-trim="false" placeholder="why oh why?" name="reason" ></textarea></div></li><li ng-show="(expensesform.reason.$dirty || expensesform.$submitted) && expensesform.reason.$error.required"><label></label><span class="error_message">Reason is required!</span></li><li><label></label> <button type="submit" ng-click="saveExpense()" class="solid">Save Expense</button> <button type="button" ng-click="clearExpense();resetFormState();">Cancel</button></li></ul></form><h3>Submitted expenses</h3><div class="list-table expenses-data"><div class="header-row"><div>Date</div><div class="figures">Amount (inc. VAT)</div><div class="figures">Reason</div><div class="figures">VAT Amount</div></div><div ng-repeat="expense in expenses" class="body-row"><div>{{expense.date | date:\'dd MMM yyyy\'}}</div><div class="figures">{{expense.amount | currency:"&pound;"}}</div><div class="figures">{{expense.reason}}</div><div class="figures">{{expense.vatAmount | currency:"&pound;"}}</div></div></div></div>')}]);
...
app.controller("ctrlExpenses", ["$rootScope", "$scope", "Restangular", function ExpensesCtrl($rootScope, $scope, $Restangular) {
	...
	$scope.calculateVAT = function(amount){
		if(amount != undefined){
			$scope.newExpense.vatAmount = amount * 1/6;
		}
		else{
			$scope.newExpense.vatAmount = 0.0;
		}
	...
	var loadExpenses = function() {
		...
		$Restangular.one("expenses-manager/expenses").getList(null,{nocache: new Date().getTime()}).then(function(expenses) {
			$scope.expenses = expenses;
			handleErrorResponse(false,"");
		}, function(response){
			handleErrorResponse(true,response.statusText);
		});
	}
	
	var handleErrorResponse = function (apiErrorState, errorcode){
		$scope.expensesform.apiError = apiErrorState;
		$scope.expensesform.errorcode = errorcode;
	}
	
	$scope.saveExpense = function() {
		if ($scope.expensesform.$valid) {
			// Post the expense via REST
			$Restangular.one("expenses-manager/expenses").post(null,$scope.newExpense).then(function() {
